...
调研目的：验证 AgentMedia 与 Postiz 的关系，并拆解它在 Skill / CLI / MCP / REST / dashboard 同层承接上的做法，补充聚星 Skill 新用户路径方案。

## 1. Fit check

AgentMedia 值得拆，但它不是聚星业务模板。

它的业务是 AI UGC 视频生成和自动发布，不是达人发现、达人分析或 influencer marketing。它对聚星的价值在于：同一团队把 Postiz 的 SaaS 发布能力旁边，又做了一个更 agent-native 的产品，把首页、Skill 页、CLI、MCP、REST、dashboard、pricing 和 Postiz 登录串起来。

一句话判断：

```text
AgentMedia 是 Postiz 同生态的 agent-native UGC 产品。
它比 Postiz 主站更直接展示 Skill / CLI / MCP / REST / dashboard 的统一承接方式，因此适合作为聚星 runtime onboarding 的结构参考。
```
...
- 每个 skill card 直接展示 credits 和预计耗时。
...
- Pricing 放在首页下方，credits 非常直观：Creator $39 / 3,900 credits，Pro $69 / 6,900 credits，Pro Plus $129 / 12,900 credits。
...
AgentMedia 的 skill list 是本次最有价值的页面模块之一。
...
- 每张卡应标清：适用场景、输入成本、预计结果、消耗额度、预计耗时、是否会创建 SaaS 对象。
...
AgentMedia 适合学 Skill / CLI / MCP / REST / dashboard 的统一承接结构。
```

聚星 `/skills` 的建议不变，但证据更强：

```text
Hero：让 AI 7×24 推进达人合作增长
Primary CTA：Add to your AI
Runtime：Codex / Claude Code / OpenClaw / Hermes
First task：creator search
Fallback：dashboard / REST API / later MCP
```

需要补强的页面模块：

- Runtime tabs：Codex、Claude Code、OpenClaw、Hermes，不把 MCP 混进第一版主线。
- Task cards：按业务任务展示，带输入、输出、额度、预计耗时、SaaS handoff。
- Quick Start：`install skill -> login -> doctor/quota/schema -> first creator search -> SaaS deep link`。
- Dashboard fallback：用户可以在 dashboard 看到最近 AI 结果、usage、billing 和失败恢复。
...
| Skill card 展示额度和耗时能降低不确定性 | `/skills` 任务卡展示预计 credit / time / output object | task card click、first task start、quota error rate |
| Dashboard run fallback 能承接不想安装 runtime 的用户 | 每张 task card 增加 dashboard fallback | dashboard run start、first result、return visit |
| 同一能力多入口能降低迷路 | Skill / CLI / REST 共享同一 task naming 和结果对象 | support issue、tool mismatch、repeat usage |
...
AgentMedia 用来学 Skill / CLI / MCP / REST / dashboard 的统一 onboarding 和 task card 组织。
```
...
- `/skills` 要补 task card 的 credit / time / output object 解释，这一点 AgentMedia 比 Postiz 更直接。
