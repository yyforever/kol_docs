...
调研目的：为聚星 Skill 新用户路径重构补充“外部 runtime scope 机制 + organization scope + API key fallback + propose / apply 分离”的机制参考，不把 sales CRM 业务域直接套到聚星。
...
- Close 是 permission / scope / organization setup 的机制参考。
- 结论只用于补聚星 Skill 的 runtime-specific setup、workspace 可见性、API key fallback 和 propose / apply 分离，不用于定义聚星新用户路径主框架。
...
- 它的 `mcp.read` / `mcp.write_safe` / `mcp.write_destructive` 是目前已拆对标中最清晰的 scope 机制之一。
...
Close 最值得聚星参考的是“让不同 runtime 在 setup 时选择 organization 与 scope，并提供 API key fallback”。它不适合作为聚星业务路径模板，也不应把 scope 层级直接做成新用户主路径；更适合补 workspace 命名、连接状态、fallback 和 propose / apply 分离。
...
### 4.2 Scope hierarchy：Close 的机制资产，不是聚星路径模型
...
- Scope 机制适合用于后台治理、管理员配置和高级接入说明，不适合成为普通新用户第一屏要理解的模型。
- 新用户路径应先解决 first value、runtime setup、workspace / quota 可见性和结果回写。
- 写操作安全应落在具体动作上，例如 preview、confirm、dashboard review、audit、revoke，而不是让用户先理解抽象层级。
- Close 的 scope 事实可以作为后续治理设计输入，但本轮不把它升级成聚星权限模型。
...
- 对管理员 / developer：可以补充 scope、workspace、quota、last used、revoke 等治理信息。

如果只用 scope 结构，新用户会知道“能不能写”，但不一定知道“先做什么任务”，也不一定能更快完成首次有效使用。
...
聚星不应把 scope 名称作为主文案。更实际的做法是在用户执行具体动作时说明：

- 当前操作哪个 workspace / brand / site。
- 这一步会不会影响外部对象。
- 是否需要预览、确认或 dashboard review。
- 失败后如何撤销、重试或联系 owner。
...
聚星应在具体业务动作中判断外部影响和确认方式，而不是按 create / update / delete 机械分级。
...
### 7.1 不把 permission scope 做成新用户主路径

Close 的 scope 机制可以进入管理员、开发者和治理说明，但不应放在聚星 Skill landing / Quick Start 的中心。当前更应该把页面空间留给：

- 这个 runtime 如何连接。
- 连接后先做哪个低摩擦任务。
- 结果保存到哪个 SaaS 对象。
- 当前 workspace、quota 和失败恢复在哪里看。
...
### 7.4 数据分析应先按业务结果拆 activation

目前聚星数据分析已经发现不同入口面会混在一起。后续可以先按业务结果拆 activation：

| Activation type | Example signal |
|---|---|
| Search / analyze activation | first successful creator search / analyze URL |
| Object handoff activation | first collection / campaign / monitor / email task created |
| Runtime setup activation | first successful runtime quota check / tool call |
| Execution activation | first confirmed export / send / apply |
| Retention activation | first return to saved object / second runtime session |
...
|---|---|---|---|
| Runtime-specific setup 能降低错配 | Codex / Claude Code / OpenClaw / Hermes Quick Start 都展示 workspace + quota + success check | setup completion、first successful call、wrong workspace issue | 维护成本增加 |
| 具体动作确认能降低误操作 | outreach / export / apply 先生成 preview 或 dashboard review，再确认执行 | preview created、confirmed action、cancel reason | 增加一步可能降低重度用户效率 |
...
| Usage report 能帮助付费升级 | usage page 按业务结果和 quota service 展示用量 | usage page return、upgrade click、quota hit | 需要和现有 Skill quota 对齐 |

## 9. 对标复盘

Close 修正了 Pin 之后的一个机制空白：Pin 的 single `mcp` scope 对用户简单，但 Close 展示了更完整的 scope、organization selection 和 fallback 方案。这些机制适合后续治理设计，不适合作为聚星新用户路径主框架。
