...
- 新用户 first run 应从低风险业务结果开始，例如 list / analyze / check status / save draft。
- send、schedule、apply、charge、confirm shipment 等高风险动作需要确认、quota、workspace、recipient 和 undo / audit。
- Tool table 不能只列函数名，应说明动作会不会影响外部对象、是否需要确认、会消耗什么额度。
...
|---|---|---|
| 高风险工具多 | `accept_collaboration` charge payment、`send_message` immediately、`confirm_shipment` | 聚星必须在具体动作处显示 confirmation / audit / 外部影响 |
...
| 动作确认能降低高风险失败 | execute 前强制确认 workspace / creator / campaign / recipient / quota | execute error rate、confirmation completion、support issue | 过多确认会降低自动化感 |
