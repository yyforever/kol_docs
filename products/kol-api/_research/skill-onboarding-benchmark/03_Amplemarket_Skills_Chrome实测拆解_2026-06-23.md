...
- 对聚星更接近 Codex / Claude Code / OpenClaw 的路径，Clay 已补充 runtime 分流和 governance 参考，Apollo 已补齐成熟 SaaS 权限 / credits 参考；后续继续拆 Salesforge / Forge MCP 和 HeyReach MCP。
...
2. Apollo MCP 已完成后续拆解，重点结论是 OAuth-first、permissions inheritance、credits / approval required、search-before-enrich、system of record 和外部 CTA 风险。
3. Salesforge / Forge MCP：适合补充 outbound email / mailbox / deliverability / campaign object。
4. HeyReach MCP：适合拆高风险写操作、LinkedIn outreach、reply sentiment 和 human-in-the-loop。
