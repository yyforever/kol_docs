...
- 但对普通用户不能只展示参数名。聚星应翻译成模式：
  - `Safe start`：只读 + core tools。
  - `Campaign builder`：草稿 / 保存类工具。
  - `Outreach execution`：需要确认的 external write。
  - `Beta tools`：仅内部或白名单。
...
| Read Only | action class：read、draft、safe_write、external_write、destructive |
...
- 聚星 Skill 的工具不应只按读写分层，还要按内容来源分层。
...
| 写操作焦虑 | `read-only=true` | 首次接入默认 read / draft 模式，可逐步升级权限 |
...
### 7.1 Runtime mode 应产品化，不只是文档参数

聚星可以把 Klaviyo 的 query parameters 翻译成用户模式：

| 聚星候选模式 | 默认能力 | 适合用户 |
|---|---|---|
| Safe start | core tools + read-only + no UGC | 首次试用、团队安全评估 |
| Research mode | creator search / profile / quota / collection read | 找达人、分析达人 |
| Draft mode | create collection draft / outreach draft / monitor draft | 开始沉淀 SaaS 对象 |
| Execution mode | save / export / send / apply，需显式确认 | 已信任并准备执行 |
| Beta mode | 新工具 / 内部工具 | 内部、白名单、早期测试 |
...
domain
object_type
action_class: read | draft | safe_write | external_write | destructive
...
-> show what next action requires higher permission
```
...
| Safe start 能降低授权焦虑 | 新用户默认 read-only + no UGC，完成 first search 后提示升级到 draft / execution | OAuth complete、first task completion、permission upgrade | 增加一步可能拖慢重度用户 |
