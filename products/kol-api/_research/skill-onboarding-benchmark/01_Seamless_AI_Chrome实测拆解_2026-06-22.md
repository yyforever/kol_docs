...
- 聚星 Skill / MCP / API 不应把风险层级作为新用户主路径，但 `send / schedule / apply` 等会产生外部影响的动作需要在具体执行处说明确认、限额、失败提示和回滚边界。
...
| 动作确认和额度解释能降低写操作焦虑 | 对外部影响动作展示确认、额度和失败原因 | write step start、write step completion、support issue |
