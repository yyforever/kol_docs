...
> 本文回答：NoxInfluencer 是否需要 MCP？如果做，怎样和现有 CLI / Skill 对齐而不互相拖累？怎样确保 ChatGPT / Claude / Cursor / VS Code 等 Remote MCP 客户端可接入？
...
- 做 **Remote MCP**，不是本地 `stdio` MCP。
- 主 endpoint 建议为 `https://mcp.noxinfluencer.com/mcp`，也可以先挂在现有域名的 `/mcp`。
- 主 transport 采用 **Streamable HTTP**；`HTTP+SSE / SSE` 只作为老客户端兼容项。
- 内部试点可以先用 Bearer API key；如果首批目标包含 ChatGPT UI / App，则 OAuth 不能等到 v1.0，至少要提前做一个最小 OAuth 连接层。
...
### 2.4 Remote MCP 兼容性确认

截至 `2026-04-29`，官方文档能支撑的兼容判断如下：

| 客户端 / 平台 | Remote MCP 支持判断 | 对我们的设计要求 |
|---|---|---|
| OpenAI Responses API | 支持 remote MCP server，配置核心是 `server_url`；支持 Streamable HTTP 或 HTTP/SSE。 | `/mcp` 必须公网 HTTPS 可访问；tool schema 要短而稳定；高风险 tool 默认 human approval / 服务端兜底。 |
| ChatGPT Apps / Developer Mode | ChatGPT App 的后端就是 MCP server；文档要求 server 可通过 HTTP/HTTPS 访问，生产环境应使用 HTTPS。 | 若要进 ChatGPT UI，需要 OAuth 或至少可被 ChatGPT developer mode 连接的认证方案；只靠用户手填内部 API key 不够稳。 |
| OpenAI Deep Research / company knowledge 类场景 | remote MCP 只接受专门的 `search` + `fetch` 只读接口，不支持任意 MCP tools。 | 如果声称兼容 ChatGPT deep research / knowledge，`search` 与 `fetch` 必须进入 v0.1；它们不等于业务 `search_creators`。 |
| Anthropic Claude API MCP connector | 支持公网 HTTP 的 Streamable HTTP / SSE remote MCP；当前主要支持 tools，不直接连接本地 stdio server。 | v0.1 应只依赖 tools，不依赖 resources / prompts；提供 allowlist toolset。 |
| Cursor | 官方文档列出 `stdio`、`SSE`、`Streamable HTTP`，其中 `SSE` 和 `Streamable HTTP` 可 remote，并可走 OAuth。 | 提供 Streamable HTTP URL；认证最好按 OAuth 2.1 / MCP Authorization 设计，避免只依赖私有 header 行为。 |
| VS Code / GitHub Copilot | 支持在 `mcp.json` 中配置 `type: "http"` 和 remote `url`。 | 提供可复制的 `mcp.json` 示例；不要要求本地 Node/CLI。 |

由此得到三个硬约束：

- **Remote-first**：Nox MCP 第一版必须是公网 HTTPS 的 remote server，不是本地 stdio 包装器。
- **Streamable HTTP first**：新实现优先 Streamable HTTP；SSE 只做兼容，不作为主路径。
- **ChatGPT 双模式区分**：普通 tool 调用和 ChatGPT deep research / knowledge 不是同一件事。前者可以暴露业务 tools，后者必须实现 `search` / `fetch`。
...
如果 ChatGPT / OpenAI Deep Research / company knowledge 是首批验证目标，下面两个 read-only tools 应进入 v0.1，而不是留到以后：
...
- `search` / `fetch` 只返回可引用、可展示、低风险的知识或账号内资源。
- `search` / `fetch` 不做达人搜索业务决策；达人搜索仍走 `search_creators`。
- Deep Research 场景要求只读，所以 approval 应可配置为 `never`；这要求服务端严格保证它们没有副作用。
...
- 对 OpenAI Responses API、Claude API、Cursor / VS Code 等可配置 token/header 的验证路径，API key 足够。
- 当前目标是验证 remote MCP tool discovery、tool call、quota、审计和错误解释能否稳定跑通。

但这里有一个产品边界：

- 如果 v0.1 的首批目标是 **ChatGPT UI / ChatGPT App**，就不能只依赖“用户手动粘贴 API key”。
- ChatGPT UI 侧更稳的正式路径是 OAuth；临时 no-auth 只能用于 mock / demo，不应用真实账号数据。
- 因此，如果 ChatGPT UI 是 P0 client，OAuth 最小闭环应前移到 v0.1.x，而不是等 v1.0。
...
- 支持 Streamable HTTP。
- 如需兼容老客户端，再补 SSE endpoint。
- 支持 Bearer API key。
- 如果 ChatGPT UI 是首批目标，提前补 OAuth 最小闭环。
- 接入现有 `kol_claw/server` `/api/v1/*`。
- 建立首批 capability contracts。
- 实现 v0.1 tool schema。
- 若 ChatGPT / Deep Research 是首批目标，实现 `search` / `fetch`。
...
- 用至少 3 个 MCP client / 模式测试：
  - OpenAI Responses API remote MCP。
  - ChatGPT developer mode 或 Deep Research `search/fetch` 路径。
  - Claude API MCP connector、Cursor 或 VS Code 中至少一个。
- 验证 tool discovery。
- 验证 search -> profile -> audience 的链路。
- 验证 `search` / `fetch` 与业务 `search_creators` 不混淆。
...
- 若 ChatGPT / Deep Research 是首批目标，包含 `search` / `fetch`。
- 若 ChatGPT UI 是首批目标，补 OAuth 最小闭环或只用 no-auth mock demo。
- 不上 marketplace。
- 不影响现有 Skill+CLI。
- 建立首批 capability contracts。
...
2. 首批 MCP client 是 ChatGPT UI、OpenAI Responses API、Claude、Cursor、VS Code、OpenClaw 还是自研 Agent。
3. 如果 ChatGPT / Deep Research 是首批目标，`search` / `fetch` 必须进入 v0.1；如果不是，是否先只做业务 tools。
4. `contacts_gated` 是否进入 v0.1，还是延后到 v0.2。
5. 如果 ChatGPT UI 是首批目标，OAuth 最小闭环是否提前到 v0.1.x。
6. 首批 capability contracts 放在 `kol_claw/contracts/`，还是独立成 `nox-capabilities` 包。
...
- OpenAI ChatGPT MCP：`https://platform.openai.com/docs/mcp`
...
- Anthropic / Claude MCP connector：`https://docs.claude.com/en/docs/agents-and-tools/mcp-connector`
