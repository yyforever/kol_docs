...
- 免费试用和自助购买都只要求注册登录，不要求 SaaS 付费会员。
...
- 支付能力尽量复用 `/product/payment/member`，但必须通过 `productType=rest_api_credit` 或等价商品类型区分，页面不得出现会员/订阅权益文案。
- API 文档继续使用 Theneo。
...
- 不再沿用“按地区把 Rest API Credit 购买资格绑定到 SaaS 付费会员”的旧规则。
...
- Rest API Credit 在 `/product/pricing` 的可发现入口。
- `/product/payment/member` 支付能力复用时的商品类型隔离。
- Theneo 文档分区方案。
...
- 明确说明 Rest API 试用和购买不要求 SaaS 付费会员。
...
目标：基于现有 `/api-service` 和新增 `/developer-api/dashboard` 完成 Rest API 用户路径、Theneo 文档入口、API Runner 链路和购买入口。

需要做：

- 改造国内/海外 `/api-service` 页面作为公开入口，不把现有 `/developer-api` landing 草稿当主入口。
- 在现有 `/api-service` 的 `API Inquiry / API Doc` 信息架构上增加免费试用和自助购买分流。
- 新增登录后 `/developer-api/dashboard`，承接 Rest API key、Credit balance、Usage、Quick Start、购买 Credit、联系销售。
...
- `/product/pricing` 新增 Rest API Credit section，但不能合并进付费会员套餐表。
- 尽量复用 `/product/payment/member` 支付页能力，使用 `productType=rest_api_credit&packageId=rest_api_1000` 或等价参数区分商品。
- 保留联系销售入口。
- 保留并强化 Theneo API 文档入口。
- 明确 API Runner 是当前 playground。
- 注册 / 登录后获取 Rest API key 的路径。
- Credit 余额、使用明细和充值入口。
- 免费试用和购买只要求登录，不要求 SaaS 付费会员。
...
- `/developer-api/dashboard` 能承接独立 Rest API key、Credit、Usage、Quick Start、购买和销售入口。
