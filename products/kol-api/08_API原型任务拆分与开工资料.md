...
- Rest API Credit 在海外英文站 `/product/pricing` 的独立 section，以及国内 / 海外购买资格展示规则；国内中文站 `/product/pricing` 不新增 Rest API Credit section。
...
- 海外英文站 `/product/pricing` 新增 Rest API Credit section，但不能合并进付费会员套餐表；国内中文站 `/product/pricing` 不新增该 section，不展示 `1000 Credit / 1.5 元` 自助包价格说明或购买 Credit CTA。
...
- 海外英文站 `/product/pricing` 有 Rest API Credit section，但不混入付费会员套餐表；国内中文站 `/product/pricing` 不新增 Rest API Credit section。
