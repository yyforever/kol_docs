...
当前公开文档基线是 `@noxinfluencer/cli` `0.4.12` 或更新版本。安装最新 CLI：
...
只看版本号不够。如果本地或全局安装残留了旧编译文件，版本输出可能看起来正常，但命令树仍然不完整。重新安装 latest 后如果 `schema --all` 仍缺少上述命令组，应停止对应工作流，并按 CLI 包或命令树不匹配处理。当前工作流还需要安装树暴露 `login`、达人搜索过滤、相似达人、邮件收件人过滤、邮件协作者、邮件 / 消息附件、商品中心、品牌监控和 feedback 等命令。
