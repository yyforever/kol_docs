...
- 访问准备：这个 Skill 应该使用哪个 NoxInfluencer 账号和 API key
...
在 OpenClaw 和其他兼容环境中，优先使用宿主安全注入或 `NOXINFLUENCER_API_KEY`。如果你需要手动配置本地 CLI，使用 `noxinfluencer auth --key-stdin`，不要把 key 放进命令参数或日志里。
...
OpenClaw 元数据要求 `NOXINFLUENCER_API_KEY` 和 `noxinfluencer` 命令可用。
...
当前公开文档基线是 `@noxinfluencer/cli` `0.4.9` 或更新版本。安装最新 CLI：
...
只看版本号不够。如果本地或全局安装残留了旧编译文件，版本输出可能看起来正常，但命令树仍然不完整。重新安装 latest 后如果 `schema --all` 仍缺少上述命令组，应停止对应工作流，并按 CLI 包或命令树不匹配处理。当前营销运营工作流还需要安装树暴露达人搜索过滤、邮件收件人过滤、邮件协作者、商品中心和品牌监控等嵌套命令。
