...
当前公开文档基线是 `@noxinfluencer/cli` `0.4.7` 或更新版本。安装最新 CLI：
...
只看版本号不够。如果本地或全局安装残留了旧编译文件，版本输出可能看起来正常，但命令树仍然不完整。重新安装 latest 后如果 `schema --all` 仍缺少上述命令组，应停止对应工作流，并按 CLI 包或命令树不匹配处理。
