...
2026-06-26 用 Chrome 复核后，目标方向不变，但需要把“参考事实”和“聚星决策”分清：

| 对象 | 实测事实 | 对聚星的约束 |
|---|---|---|
| AgentMedia 首页 | 首屏直接卖 UGC 结果，并给出安装命令 | 学“可执行优先”，不照搬首屏命令堆叠 |
| AgentMedia `/skills` | 默认展示 Skill tab，MCP / CLI / Skill 分层，Quick Start 是三步 | 聚星 `/skills` 也要三步承接，但第一版不把 MCP 放进主路径 |
| AgentMedia skill cards | 展示任务、credits、耗时和 dashboard run | 聚星只学任务卡结构，不展示额度，不放 dashboard run |
| Postiz 首页 | 首屏强调 agentic social scheduling，主 CTA 是付费试用 | 学强业务承诺和 SaaS 闭环，不学付费阻塞 |
| Postiz agent 页 | 页面很 CLI-heavy，依赖 API key | 不作为聚星普通用户首轮路径模板 |
| 聚星 `/skills` 线上 | 当前是 marketing landing：GitHub / Skills / ClawHub / Quick Start 并列，含 pricing 和 ChatGPT | P0 必须改成 Skill Center，并把 `Add to your AI` 作为唯一主 CTA |
| 聚星 docs | Quick Start 已覆盖完整安装、登录、API key fallback、MCP 等细节 | docs 是 fallback/detail，不是首轮激活承接页 |
| 聚星 dashboard | 默认是 API Keys 与 Usage & Billing | dashboard 只做结果管理、历史、失败恢复和额度场景 |
| skills.sh / ClawHub | skills.sh 给通用 install；ClawHub 给 OpenClaw 专用安装入口 | Runtime picker 要分别使用正确入口，不能混用 URL 和命令 |
...
本节只记录需要改掉的线上差距，不作为目标路径长期保留。

| 页面 | 当前可见状态 | 目标处理 |
|---|---|---|
| `/skills` 首屏 | H1 是 `Influencer Marketing Powered by AI Agents`，CTA 是 GitHub / Skills / ClawHub / Quick Start | 改为一个主 CTA：`Add to your AI`，runtime 选择进入主体区域 |
| `/skills` runtime | 展示 ChatGPT / Claude / OpenClaw，缺少 Codex / Claude Code / Hermes 选择 | 第一版只保留 Codex、Claude Code、OpenClaw、Hermes；ChatGPT 后续单独做 connector |
| `/skills` 内容顺序 | 痛点、assistant、pricing、FAQ 更像普通 landing | 改为 Skill Center：Hero -> runtime picker -> Quick Start -> first task -> task cards |
| `/skills/docs` | 是文档索引，包含 MCP、Developer API、API key fallback、diagnostics | 保持文档入口，但不让它承担首轮转化路径 |
| `/skills/docs/getting-started/quick-start/index` | 是完整安装文档，含 public entry points、dashboard/API key fallback、ChatGPT status、diagnostics | landing 只抽取三步；完整文档作为 `View install details` |
| `/skills/dashboard` | 默认显示 API Keys、Usage & Billing、quota、recent activity、Quick Start | 作为 fallback 和结果管理，不作为 task card CTA |
...
```text
H1: 让你的 AI 跑完整达人营销
Subcopy: 把 NoxInfluencer 加到 Codex、Claude Code、OpenClaw 或 Hermes。你的 AI 可以搜索达人、判断合作优先级、沉淀名单、准备跟进，并把结果带回聚星继续管理。
...
```text
Let your AI run creator marketing from discovery to follow-up.
```
