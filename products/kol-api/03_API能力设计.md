...
**NoxInfluencer 设计目标：7 个 Tool 覆盖完整路线图。Day 1 先上线 4 个核心 Tool（discover / analyze / track / manage 只读），把搜索、深度评估和监控做成可商业化、可防搬数的最小闭环；执行自动化能力在后续版本按调用数据再上。**
...
# analyze_creator
nox analyze crt_7fd91a --deep

# track_performance
nox track crt_7fd91a --window 30d
nox track crt_7fd91a --metrics followers,views,engagement --window 90d
...
| **v1.1**（1 个新 + 1 个增强） | competitive_intel, manage_campaigns 增强版（加写操作） | 在 Day 1 数据跑通后增加高信息密度洞察和更强的上下文管理 |
...
| `niche` | | string | — | 内容品类 |
| `count` | | int | 10 | 返回数量（最大 10，超过返回 400） |
| `cursor` | | string | — | 下一页游标 |

**返回**：排序的候选达人清单，每人只含 `creator_id` + 不可外链摘要字段：平台、国家、品类、粉丝区间、互动率、粗粒度真实性、预估合作费区间。**不返回** `handle`、频道链接、平台 channel id、头像 URL、display name 等可直接关联外部平台的数据。

**关键设计**：
1. **1 credit = 1 页候选达人**，每页最多 10 个 creator，Starter 相当于平均每个 candidate creator 约 `$0.0048`
2. 搜索的目标是**粗筛 + 排序**，不是低价导出联系人库
3. 想看频道链接、详细受众和真实性明细，必须继续调 `analyze_creator`
...
**关键设计**：`analyze_creator` 是公开外链数据和深度分析数据的唯一出口。搜索结果只提供内部 `creator_id`，从接口层强制建立 `discover -> analyze` 的付费路径。

### 3.3 `outreach_creators` — "帮我联系这些达人"

**优先级**：P2（v1.2） | **Credit**：20 credits/人

**定位**：执行自动化能力，待 Day 1 的搜索/分析/监控调用数据验证后再上。

### 3.4 `negotiate` — "帮我谈价"

**优先级**：P2（v1.2） | **Credit**：10 credits/轮
...
**优先级**：P0 | **Credit**：10 credits/次

**合并 02 能力**：`get_creator_stats` + `get_campaign_performance`

| 参数 | 必填 | 类型 | 说明 |
|------|:----:|------|------|
| `creator_id` | ✅ | string | 达人 ID（必须来自 discover / manage_campaigns） |
| `window` | | enum | 30d | 7d / 30d / 90d |
| `metrics` | | array | 指定指标（followers / views / engagement / posting_frequency） |

**返回**：频道粉丝趋势 + 播放趋势 + 互动率变化 + 发帖频率 + 异常波动提示 + 与同量级同品类基准对比。

**关键设计**：监控不是 Campaign 结案页，而是品牌持续跟踪频道表现的高频工具。Day 1 就应支持。
...
```json
{
  "success": true,
  "data": { ... },
  "summary": "找到 10 位 US 美妆 TikTok 候选达人。Top 3 creator_id：crt_7fd91a（4.2%）、crt_20b4c3（3.8%）、crt_91af2d（3.5%）。可进一步调用 analyze_creator 查看频道链接和完整受众。（as of 2026-03-09）",
  "credits": {
    "used": 1,
    "remaining": 29,
    "plan": "free"
  ...
  "meta": {
    ...
    "latency_ms": 1200,
    "data_freshness": "2026-03-09T10:00:00Z"
  }
}
```
...
```json
{
  ...
  "error": {
    ...
    "message": "本次操作需要 5 credits，当前余额 3 credits。升级到 Starter（$29/月）获得 600 credits。",
...
|------|:---------:|------------|------|
| `discover_creators` | 1 | 达人搜索 | 1 页候选达人（最多 10 个内部 ID） |
| `analyze_creator` | 5 | 达人详情+受众 | 深度分析单个达人 |
| `outreach_creators` | 20/人 | 邮件邀约 | v1.2；执行自动化能力 |
| `negotiate` | 10/轮 | AI 谈判（每轮） | v1.2；执行自动化能力 |
| `manage_campaigns` | 0 | — | 免费的长期记忆层和上下文容器 |
| `competitive_intel` | 20 | 竞品对标 | 高信息密度洞察 |
| `track_performance` | 10 | 频道效果追踪 | 高价值监控与复盘 |
...
| 评估 5 | 达人详情+受众 5 credits | analyze_creator 5 | ✅ |
| 监控 10 | 效果追踪 10 credits | track_performance 10 | ✅ |
| 邀约 20 | 邮件邀约 20 credits | outreach_creators 20/人 | ✅ |
| 谈判 10 | AI 谈判（每轮）10 credits | negotiate 10/轮 | ✅ |
| — | 竞品对标 20 credits | competitive_intel 20 | ✅ |

### 5.3 典型使用场景的 Credit 消耗

| 场景 | 操作链 | Credit 消耗 | Free 层（30）够用？ |
|------|--------|:----------:|:-----------------:|
| 试用：找达人看看 | discover×5 + analyze×3 | 20 | ✅ |
| 试用：找人并看一次监控 | discover×3 + analyze×3 + track×1 | 28 | ✅ |
| 月度轻度研究 | discover×40 + analyze×20 + track×10 | 240 | 需 Starter |
| 月度常规（Starter $29） | discover×120 + analyze×60 + track×18 | 600 | ✅ 刚好 |
| 竞品研究 | competitive_intel×4 + analyze×8 | 120 | ✅ |
| 执行自动化（v1.2） | outreach×10 + negotiate×3 | 230 | 需付费层 |
...
| D8 | 发布节奏 | Day 1 上 4 个（discover/analyze/track/manage 只读），v1.1 加 1 个新 + 1 个增强，v1.2 再上执行自动化 | 7 个同时上线 | Firecrawl 模式：带最小集上线，用调用数据驱动迭代。先把防搬数、计费和监控闭环跑通，再决定是否加执行自动化 |
...
| 核心场景 | 搜索 / 评估 / 监控优先 | 8 个场景（3.1-3.8） | 7 个 Tool = Day 1 上 4 个 + v1.1 新增 1 个 + v1.2 新增 2 个（manage_campaigns 同步增强，不计新 Tool） | ✅ |
| Credit 定义 | 搜索 1 / 评估 5 / 监控 10 | 同 01/02 | 完全一致（第五节） | ✅ |
...
以下 7 段描述可直接用于 MCP metadata 中的 `description` 字段。Day 1 发布 4 个（discover / analyze / track / manage），v1.1 增加 `competitive_intel` 和 `manage_campaigns` 增强版，v1.2 再增加执行自动化能力。
...
**analyze_creator**
> Get a deep analysis of a specific creator identified by creator_id from discover_creators. Returns the direct profile link, handle, authenticity scoring, audience demographics, content performance trends, and estimated pricing. Use this tool when a brand wants to decide whether a creator is trustworthy, relevant, and worth contacting or tracking.
