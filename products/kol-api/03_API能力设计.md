...
Day 1 的 manage_campaigns 支持创建 Campaign（定义营销目标和约束，作为搜索/监控上下文）+ 只读查询。创建和查询都免费（0 credit），让 Agent 更愿意先查历史、建 watchlist、再做分析。写操作（set_alert / update_status）留到 v1.1 增强版。
...
**优先级**：P0 | **Credit**：默认 1 credit/次（`protected`）
...
**关键设计**：
1. **1 credit = 1 页默认保护搜索**，每页最多 20 个 creator；`basic_info` 档位按 4 credits / 页计费
2. 搜索的目标是**粗筛 + 排序**，不是低价导出联系人库
3. `基础信息` 指 URL、外部频道 ID 等可关联字段；默认档不返回
4. 想看完整受众、内容分析、品牌分析和结构化深度结论，必须继续调 `analyze_creator`
5. Free 可以使用 `basic_info`，但要走更严格的 route-specific rate limit

### 3.2 `analyze_creator` — "这个达人靠谱吗"

**优先级**：P0 | **Credit**：按维度计费（全维度 1.7 credits）
...
**返回**：
- `overview`（0.4 credit）：频道链接 / 平台外部 ID / `handle` / 显示名称 / 基础表现
- `audience`（0.5 credit）：受众国家、年龄、性别、兴趣等画像
- `content`（0.4 credit）：近期内容表现、发文结构、趋势总结
- `brand`（0.4 credit）：品牌契合度、赞助内容线索、商业合作判断
- 默认全选时返回以上四个维度（合计 1.7 credits）

**关键设计**：
1. `analyze_creator` 是**深度数据的主出口**；默认搜索只暴露内部 `creator_id`
2. 深度数据按 `数据总览 / 粉丝受众 / 内容分析 / 品牌分析` 四个栏目切分，支持按需付费
3. 每个模块的对外 Credit 定价都控制在"最便宜一次保护搜索"的约三分之一到二分之一之间，同时高于内部风控下限
4. 如果用户只想拿可关联字段，最便宜的安全路径是 `discover_creators.basic_info`；如果要深度判断，走 `analyze_creator`

### 3.3 `outreach_creators` — "帮我联系这些达人"

**优先级**：P2（v1.2） | **Credit**：10 credits/人

**定位**：执行自动化能力，待 Day 1 的搜索/分析/监控调用数据验证后再上。

### 3.4 `negotiate` — "帮我谈价"

**优先级**：P2（v1.2） | **Credit**：5 credits/轮

**定位**：执行自动化能力，运营和合规复杂度高，放到 v1.2。

### 3.5 `manage_campaigns` — "管理我的合作"

**优先级**：P1（Day 1 支撑能力）| **Credit**：创建/查询均为 0 credit
...
**关键设计**：创建和查询都免费（0 credit），把 Campaign 作为搜索偏好、watchlist 和后续监控的上下文容器；付费集中在深度分析、洞察与效果追踪。
...
**优先级**：P0 | **Credit**：`start` = 1.5 credits / video / 15 天；`query` = 0 credit
...
**关键设计**：
1. 监控不是一次性查询，而是**按视频、按 15 天周期收费**的持续跟踪能力；创建任务时立即扣费
2. 查询已创建监控任务的结果不收费，但仍受基础 rate limit 约束
...
**优先级**：P1（v1.1） | **Credit**：2 credits / creator
...
**关键设计**：
1. 联系方式是单独商品，不和普通深度分析打包
2. Day 1 不上线，但定价和文档口径提前统一，避免后续收费项和 Tool 设计脱节
...
```json
{
  "success": true,
  "data": { ... },
  "summary": "找到 20 位 US 美妆 TikTok 候选达人。Top 3 creator_id：crt_7fd91a（4.2%）、crt_20b4c3（3.8%）、crt_91af2d（3.5%）。如需频道链接、外部 ID 或完整深度数据，可继续调用 analyze_creator。（as of 2026-03-10）",
  "credits": {
    "used": 1,
    "remaining": 9,
    "plan": "free"
  ...
  "meta": {
    ...
    "latency_ms": 1200,
    "data_freshness": "2026-03-10T10:00:00Z"
  }
}
```
...
```json
{
  ...
  "error": {
    ...
    "message": "本次操作需要 1.7 credits，当前余额 0.8 credits。升级到 Starter（￥290 / $29 / 月）获得 200 credits。",
...
| 余额不足 | "需要 1.7 credits，余额 0.8。品牌可在 noxinfluencer.com 升级" | 给出行动路径 |
...
|------|:---------:|------------|------|
| `discover_creators`（`protected`） | 1 | 达人搜索 | 1 页候选达人（最多 20 个内部 ID） |
| `discover_creators`（`basic_info`） | 4 | 搜索含基础信息 | 返回可外部关联字段的高价档位 |
| `analyze_creator.overview` | 0.4 | 数据总览 | 解锁外链字段 + 基础表现 |
| `analyze_creator.audience` | 0.5 | 粉丝受众 | 四模块中最贵 |
| `analyze_creator.content` | 0.4 | 内容分析 | 近期内容表现和趋势 |
| `analyze_creator.brand` | 0.4 | 品牌分析 | 真实性和品牌契合判断 |
| `analyze_creator`（全维度） | 1.7 | 达人详情+受众 | 0.4+0.5+0.4+0.4 的四维度组合 |
| `track_performance.start` | 1.5 / video / 15 天 | 效果监控启动 | 创建监控时按视频数和周期数扣费 |
| `track_performance.query` | 0 | 效果监控查询 | 查询已创建监控任务结果免费 |
| `get_contacts` | 2 / creator | 联系方式解锁 | v1.1；邮箱 / 联系方式独立收费 |
| `outreach_creators` | 10/人 | 邮件邀约 | v1.2；执行自动化能力 |
| `negotiate` | 5/轮 | AI 谈判（每轮） | v1.2；执行自动化能力 |
| `manage_campaigns` | 0 | — | 免费的长期记忆层和上下文容器 |
| `competitive_intel` | 10 | 竞品对标 | 高信息密度洞察 |

### 5.2 与 01/02 一致性对齐

| 01 第七节 Credit 定义 | 02 第 2.4 节 Credit 定义 | 03 Tool 映射 | 一致性 |
|---------------------|------------------------|-------------|:------:|
| 搜索 1 / 基础信息搜索 4 | 达人搜索 | discover_creators 1 / 4 | ✅ |
| 评估模块 0.4-0.5 / 全维 1.7 | 达人详情+受众 | analyze_creator 0.4+0.5+0.4+0.4 | ✅ |
| 监控启动 1.5 / 查询 0 | 效果追踪（15 天） | track_performance start/query | ✅ |
| 联系方式 2 | 邮箱解锁 | get_contacts 2 | ✅ |
| 邀约 10 | 邮件邀约 | outreach_creators 10/人 | ✅ |
| 谈判 5 | AI 谈判（每轮） | negotiate 5/轮 | ✅ |
| — | 竞品对标 10 credits | competitive_intel 10 | ✅ |

### 5.3 典型使用场景的 Credit 消耗

| 场景 | 操作链 | Credit 消耗 | Free 层（10）够用？ |
|------|--------|:----------:|:-----------------:|
| 试用：1 次完整闭环 | discover×1 + analyze(全维)×1 + track.start(2 videos)×1 | 5.7 | ✅ |
| 试用：多看几个达人 | discover×2 + analyze(全维)×2 + track.start(2 videos)×1 | 8.4 | ✅ |
| 月度轻度研究 | discover×30 + analyze(全维)×20 + track.start(20 videos)×1 | 94 | 需 Starter |
| 月度常规（Starter `￥290 / $29`） | discover×50 + analyze(全维)×40 + track.start(30 videos)×1 | 163 | ✅ |
| 竞品研究 | competitive_intel×4 + analyze(全维)×8 | 53.6 | ✅ |
| 执行自动化（v1.2） | outreach×10 + negotiate×3 | 115 | 需付费层 |
...
| D7 | Credit 映射 | 与 01/02 完全一致，并受 04 风控下限约束 | 重新定义 | 保持三份文档一致性 |
...
| # | 问题 | 当前选择 | 待验证 |
|---|------|---------|--------|
| T1 | **MCP Resource vs Tool 分类** | 全部实现为 Tool | 需调研 MCP 协议规范中 Resource 和 Tool 的实际区分标准。analyze_creator / competitive_intel / `track_performance.query` 是否应作为 Resource 实现（只读、无副作用、无 Credit 消耗门槛）？参考 Firecrawl / Context7 / Supabase MCP 的实践选择 |
...
| Credit 定义 | 搜索 1 / 评估模块 0.4-0.5 / 监控启动 1.5 / 联系方式 2 | 同 01/02 | 完全一致（第五节） | ✅ |
