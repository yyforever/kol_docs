...
Day 1 的 manage_campaigns 支持创建 Campaign（定义营销目标和约束，作为全流程上下文）+ 只读查询。创建和查询都免费（0 credit），让 Agent 更愿意先查历史再行动。写操作（set_alert / update_status）留到 v1.1 增强版。
...
**优先级**：P0（Day 1）| **Credit**：创建/查询均为 0 credit
...
**关键设计**：创建和查询都免费（0 credit），把 Campaign 作为全流程上下文容器和品牌长期记忆层；付费集中在深度分析、邀约、洞察与效果追踪。
...
| `outreach_creators` | 10/人 | 邮件邀约 | 含联系方式查询+邮件发送+追踪 |
| `negotiate` | 5/轮 | AI 谈判（每轮） | 含市场基准+自动邮件往返 |
| `manage_campaigns` | 0 | — | 免费的长期记忆层和上下文容器 |
| `competitive_intel` | 15 | 竞品对标 | 品牌合作历史分析 |
| `track_performance` | 10 | — | 高价值效果归因与 ROI 复盘 |
...
| 邀约 10 | 邮件邀约 10 credits | outreach_creators 10/人 | ✅ |
| 谈判 5 | AI 谈判（每轮）5 credits | negotiate 5/轮 | ✅ |
| — | 竞品对标 15 credits | competitive_intel 15 | ✅ |

### 5.3 典型使用场景的 Credit 消耗

| 场景 | 操作链 | Credit 消耗 | Free 层（200）够用？ |
|------|--------|:----------:|:------------------:|
| 试用：找达人看看 | discover×3 + analyze×5 | 28 | ✅ 够 7 轮 |
| 小型 Campaign | discover + analyze×10 + outreach×10 | 151 | ✅ 够 1 轮 |
| 完整 Campaign | discover + analyze×10 + outreach×10 + negotiate×5 | 176 | ✅ 够 1 轮 |
| 竞品研究 | competitive_intel×2 + analyze×3 | 45 | ✅ 够 4 轮 |
| CRM 日常（月度） | manage×20 + track×5 | 50 | ✅ 够 4 轮 |
| 月度常规（Starter $29） | 上述×5 + competitive_intel×2 + manage×10 + track×5 | ~960 | 需 Starter |
...
| 核心场景 | 7 个（P0×3 + P1×3 + P2×1） | 8 个场景（3.1-3.8） | 7 个 Tool = Day 1 上 5 个 + v1.1 新增 2 个（manage_campaigns 同步增强，不计新 Tool） | ✅ |
