...
**优先级**：P0 | **Credit**：按维度计费（全维度 5 credits）
...
**返回**：
- `overview`（1 credit）：频道链接 / 平台外部 ID / `handle` / 显示名称 / 基础表现
- `audience`（2 credits）：受众国家、年龄、性别、兴趣等画像
- `content`（1 credit）：近期内容表现、发文结构、趋势总结
- `brand`（1 credit）：品牌契合度、赞助内容线索、商业合作判断
- 默认全选时返回以上四个维度（合计 5 credits）

**关键设计**：
1. `analyze_creator` 是**深度数据的主出口**；默认搜索只暴露内部 `creator_id`
2. 深度数据按 `数据总览 / 粉丝受众 / 内容分析 / 品牌分析` 四个栏目切分，支持按需付费
3. 如果用户只想拿可关联字段，最便宜的安全路径是 `discover_creators.basic_info`；如果要深度判断，走 `analyze_creator`
...
**优先级**：P0 | **Credit**：3 credits / creator / 15 天
...
| `window` | | enum | 15d | Day 1 默认按 15 天监控周期计费 |
| `metrics` | | array | 指定指标（followers / views / engagement / posting_frequency） |

**返回**：频道粉丝趋势 + 播放趋势 + 互动率变化 + 发帖频率 + 异常波动提示 + 与同量级同品类基准对比。

**关键设计**：
1. 监控不是一次性查询，而是**按 creator、按 15 天周期收费**的持续跟踪能力
2. 这是 Day 1 的核心留存能力，优先级与搜索、详情并列
3. 监控只接受内部 `creator_id`，不接受外部 URL 直查
...
```json
{
  ...
  "error": {
    ...
    "message": "本次操作需要 5 credits，当前余额 3 credits。升级到 Starter（￥290 / $29 / 月）获得 200 credits。",
...
| `analyze_creator`（全维度） | 5 | 达人详情+受众 | 1+2+1+1 的四维度组合 |
...
| `track_performance` | 3 | 频道效果追踪（15 天） | 高价值监控与复盘 |
...
| 评估 5 | 达人详情+受众 | analyze_creator 5（1+2+1+1） | ✅ |
| 监控 3 | 效果追踪（15 天） | track_performance 3 | ✅ |
...
|------|--------|:----------:|:-----------------:|
| 试用：1 次完整闭环 | discover×1 + analyze(全维)×1 + track×1 | 9 | ✅ |
| 试用：多看几个达人 | discover×2 + analyze(全维)×1 + track×1 | 10 | ✅ 刚好 |
| 月度轻度研究 | discover×30 + analyze(全维)×20 + track×10 | 160 | 需 Starter |
| 月度常规（Starter `￥290 / $29`） | discover×50 + analyze(全维)×20 + track×15 | 155 | ✅ |
| 竞品研究 | competitive_intel×4 + analyze(全维)×8 | 80 | ✅ |
