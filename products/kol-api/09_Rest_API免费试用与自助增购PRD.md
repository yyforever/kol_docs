...
- `/developer-api` 不是当前公开入口，不应进入本 PRD 的用户路径。
...
| 试用路径 | 当前没有 Rest API 免费试用入口 | 访客点击试用后注册 / 登录，进入 Rest API Console 开通免费试用 |
| 购买路径 | 当前没有 Rest API Credit 自助购买入口 | 登录用户可购买 Rest API Credit，不要求付费会员 |
| 登录后承接 | 当前没有 API key、Credit、usage、Quick Start 的统一承接页 | 新增 Rest API Console，集中承接 key、Credit、usage、Quick Start、购买和销售入口 |
...
- 如果该用户已经有 Skill 权益，账号内页面必须把 Skill 次数和 Rest API Credit 分开展示；Rest API 详情仍进入 Rest API Console。
...
| Rest API Console | 当前没有 | 新增登录后承接页，展示 API key、Credit、usage、Quick Start、购买入口、销售入口 | 必须登录 |
| 免费试用开通模块 | 当前没有 | 新增开通动作，发放免费 Credit，并进入 Quick Start | 必须登录，不要求付费会员 |
| Rest API Credit 购买页 / 模块 | 当前没有 | 新增 `1000 Credit` 起售包购买入口和说明 | 必须登录，不要求付费会员 |
| Usage 明细模块 | 当前没有 | 新增 Rest API 调用明细，只展示 Rest API Credit 消耗 | 必须登录 |
...
点击 购买 Credit
  ...
  -> 完成购买
  -> Credit 余额更新
  -> 返回 Rest API Console / API Explorer
```
...
Rest API Console 是登录后承接页，不能和 Skill 权益页混写。
...
- 显示预计响应时间。
- 引导用户先查看文档或试用基础接口。
- 如果用户已登录，可回 Rest API Console 查看当前状态。
...
| 免费用户，未开通试用 | 可直接开通 Rest API 免费试用 | 开通试用 |
...
- 用户能从 Rest API Console 进入 Quick Start、Theneo docs 和 API Explorer。
- 用户在 Credit 不足时能进入购买路径。
- 用户在需要搜索或高级接口时能进入销售路径。

### 15.2 信息一致性验收

- 页面、文档、Console 中均使用 Rest API Credit，不出现 Skill 次数混用。
...
- Rest API Console 已能展示 API key、Credit、usage、Quick Start、购买入口和销售入口。
- 购买 Credit 流程不要求付费会员。
- 销售表单能承接大额 / 定制接口。
...
2. 免费试用到期或用尽后的标准升级文案。
3. 大额 / 定制接口的转人工门槛。
4. Rest API Console 的最终页面路径和导航位置。
5. 自助购买 Credit 的支付入口和发票承接方式。
