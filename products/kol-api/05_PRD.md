...
| **摩擦 🟡** | 不知选哪档 → pricing 页必须解释 AI创业版 / 企业版 / 高级企业版差异 |
...
| `track_performance` | 持续监控表现 | 1 / 次 | 按实际发生校验与扣减 | 0 | 付费升级驱动力 |
...
3. `start/query` 都作为一次 Tool 调用，均消耗 1 次 Skill 技能额度。
4. 底层监控类服务配额是否扣减，以本次实际发生为准。
...
| AI创业版 | 月付 | 否 | 开放 Skill 能力，额度按月重置 |
| 企业版 | 季付起 | 是 | 开放 Skill 能力，额度按月重置 |
| 高级企业版 | 年付 | 是 | 更高 Skill 能力和更多服务配额 |
...
| `track_performance` | 1 次 Skill 技能额度 + 监控类服务配额 |
...
**页面必须体现**：
- 套餐梯度：免费 / AI创业版 / 企业版 / 高级企业版 / 定制版
- Skill 技能是已有套餐能力之一
- 免费用户一次性试用：搜索 10 次、查看 30 次、监控 0
- 付费用户 Skill 技能额度按月重置
- “具体服务配额消耗以实际发生为准”
...
| `SkillQuotaService` | 除 `manage_campaigns` 外 | 扣减 Skill 技能额度 |
| `ServiceQuotaAdapter` | 全部 Tool | 校验并执行底层服务配额扣减 |
...
- `track_performance` 的 `start/query` 都走同一套 Skill 技能额度校验
...
| D7 | 除 `manage_campaigns` 外，Day 1 Tool 暂按每次调用消耗 1 次 Skill 技能额度 |
...
**track_performance**
> Start or query a creator monitoring job for recent performance trends. Returns follower growth, view and engagement changes, anomaly flags, and peer benchmarks. Use this when a brand already has shortlisted or active creators and needs ongoing visibility into performance changes.
