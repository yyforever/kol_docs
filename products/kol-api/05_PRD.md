...
| **激活指标** | **首次将达人加入待邀约列表**（outreach_creators preview 被调用）——搜索结果质量够好，品牌愿意进入下一步 |
...
| **摩擦 🟡** | 达人回复管理——outreach 发了邮件，达人回了怎么知道？→ Day 1 邮件通知 + manage_campaigns 状态更新（TODO：邮件通知模板） |
| **失败→恢复** | Credit 消耗完但未形成习惯→放弃 → Credit 低时发价值回顾邮件"你已找到 N 个达人" |
| **产品需要** | manage_campaigns 完整覆盖合作状态；Tool 返回 summary 供 Agent 存入记忆；达人回复通知（TODO） |
...
| **产品需要** | 定价页 + Credit 模拟器 + Stripe Checkout + credit 低提醒邮件（TODO） |
...
| `confirm: true`（启动谈判） | 所有层级均可用，扣 5 credits——Credit 配额是唯一限制（LLM 推理成本由用户 Agent 平台承担） |
...
- [ ] `confirm: true` 所有层级正常扣 5 credits（Credit 余额不足时返回 402）
...
| Q5 | negotiate 层级门控 | 所有层级可用，Credit 配额限制 | ✅ 已决策：LLM 推理成本由用户 Agent 平台承担，Credit 配额即天然限制 | ✅ 已决策 |
