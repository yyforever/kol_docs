...
| `outreach_creators` | 10/人 | `confirm: true` 发送成功后按实际发送人数扣减 |
...
- **同步操作**（discover / analyze）：业务操作与 Credit 扣减在同一数据库事务中，操作失败自动回滚
...
| 深度分析 | 5 | 单个达人的完整画像 + 真实性评分 |
| 邮件邀约 | 10/人 | 个性化邮件 + 自动 follow-up |
| AI 谈价 | 5/轮 | 每轮谈判（典型 3 轮 = 15 credits） |
| 查看合作 | 0 | 查看进行中的合作状态（免费） |
| 创建 Campaign | 0 | 定义营销目标和约束（免费） |
| 效果追踪 | 10 | 跨平台效果归因 + ROI 基准对比 |
| 竞品对标 | 15 | 品牌合作历史与达人池分析 |
...
───────────────────────────────────────
Estimated monthly usage: ~5,400 credits
