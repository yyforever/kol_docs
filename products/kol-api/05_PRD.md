...
| 维度 | 内容 |
|------|------|
| **行为** | Google / GitHub 一键注册（10 秒）→ Dashboard + API Key + 10 credits → 按教程配置 Agent |
...
| **产品需要** | discover + analyze + track 三个 Tool 质量过关；Free 层 10 credits 够 1 次完整 research loop；summary 字段高质量 |
...
| **摩擦 🟡** | 价格锚定——对比手动成本（$0）vs. `￥290 / $29 / 月` → 展示 ROI（节省 X 小时） |
...
| 付费转化 | 品牌觉得 `￥290 / $29 / 月` 太贵 | 中 | ROI 回顾邮件 + A/B 定价 |
...
**Credit**：默认 1 credit/次（`protected`） | **HTTP**：`POST /v1/tools/discover_creators` | **CLI**：`nox search`
...
```json
{
  ...
  "properties": {
    ...
    "result_mode": {
      "type": "string",
      "enum": ["protected", "basic_info"],
      "default": "protected",
      "description": "protected=默认保护搜索；basic_info=返回可外部关联的基础信息"
    },
    "cursor": {
      "type": "string",
...
```json
{
  ...
  "data": {
    ...
    "total_matched": 47,
    "result_mode": "protected",
    "next_cursor": "cur_eyJwYWdlIjoyfQ"
  },
  "summary": "找到 47 位 US 美妆 TikTok 候选达人，本次返回前 20 位 protected 结果。Top 3：crt_abc123（4.2%）、crt_def456（3.8%）、crt_ghi789（3.5%）。如需频道链接、外部 ID 或完整深度数据，可继续调用 analyze_creator。（as of 2026-03-10）",
  "credits": { "used": 1, "remaining": 9, "plan": "free" },
  "meta": { "request_id": "req_abc123", "latency_ms": 1200, "data_freshness": "2026-03-10T10:00:00Z" }
}
```
...
4. `result_mode=protected` 时，每位达人仅包含：内部 `creator_id` + 平台 + 国家 + 品类 + 粉丝区间 + 粗粒度真实性 + 互动率 + 预估合作费区间
5. `result_mode=protected` **不返回** `handle`、`display_name`、`avatar_url`、频道链接、外部 channel id 等可外链字段
6. `result_mode=basic_info` 是同一 Tool 的高价档位，可返回 URL / 外部 ID 等基础信息，但不返回完整深度分析
7. 想看完整画像、受众和结构化品牌 / 内容判断，必须继续调用 `analyze_creator`
8. 支持分页：返回 `next_cursor`，Agent 可传入 `cursor` 获取更多结果（`protected` 每页消耗 1 credit，`basic_info` 每页消耗 4 credits）。Agent 可自然对话："还有更多吗？" → 自动翻页
...
| `count` > 20 | 返回 400 `too_many_results_requested` |
...
| `insufficient_credits` | 402 | "当前搜索需要 1-4 credits，余额不足。升级到 Starter（￥290 / $29 / 月）获得 200 credits" | 给出行动路径 |
...
```json
{
  ...
  "properties": {
    ...
    "modules": {
      "type": "array",
      "items": { "type": "string", "enum": ["overview", "audience", "content", "brand"] },
      "description": "按需请求维度；默认全选"
    }
  }
}
```
  ...
  "data": {
    ...
    "modules": {
      "overview": {
        ...
        "profile_url": "https://www.tiktok.com/@beautybyjess",
        "external_channel_id": "tt_91827364",
        "followers": 123000,
        "engagement_rate": 0.042,
        "estimated_cost": {
          "min": 800,
      ...
      "brand": {
        ...
        "brand_fit": {
          "score": 82,
          "summary": "受众与美国 18-34 岁女性美妆用户高度重合"
        }
      }
    }
  },
...
| credit 不足（< 所请求维度消耗） | 返回 402 `insufficient_credits` |
...
| `insufficient_credits` | 402 | "所请求维度需要 {needed} credits，余额 {remaining}。升级获取更多额度" |

**验收标准**

- [ ] 通过 `creator_id` 查询返回完整画像
- [ ] 真实性评分返回 0-100 分值 + fake_follower_pct + suspicious_signals
- [ ] `modules` 缺省时返回四个维度；单独请求时只返回所选维度
- [ ] 响应时间 < 5 秒（P95）
- [ ] 返回格式符合统一信封规范
...
**Credit**：10 credits/人 | **HTTP**：`POST /v1/tools/outreach_creators` | **CLI**：`nox outreach`
...
```json
{
  ...
  "data": {
    ...
    "total_cost_credits": 120,
    "contactable": 12,
    "not_contactable": 0
  },
  "summary": "已准备 12 封个性化邀约邮件，预估响应率 15-25%，确认发送？",
  "credits": { "used": 0, "remaining": 200, "plan": "starter" },
...
```json
{
  ...
  "credits": { "used": 120, "remaining": 80, "plan": "starter" },
...
2. **发送阶段**（`confirm: true`）：品牌确认后发送邮件。**扣 credit（10/人）。** 返回发送状态 + tracking ID。3 天无回复自动发 follow-up（可关闭）。
...
| Free 层调用 `confirm: true`（发送） | 返回 403 `upgrade_required`（发送邮件仅付费层可用） |
...
| `upgrade_required` | 403 | "发送邮件需要 Starter 套餐（￥290 / $29 / 月）。Free 层可预览邮件内容（confirm: false），发送需升级" |
| `insufficient_credits` | 402 | "邀约 {n} 人需要 {n×10} credits，余额 {remaining}" |
...
```json
{
  ...
  "credits": { "used": 0, "remaining": 10, "plan": "free" },
...
| **年付** | Stripe Price 配置年付选项，全档 ~17% off（整数定价：`￥2,900 / $290`、`￥9,900 / $990`、`￥19,900 / $1,990`） |
...
**Free 层超额行为**：返回 402 `insufficient_credits` + `upgrade_url`，不允许超额计费。Free 层 10 credits 用完即止。
...
SaaS 订阅与 API / MCP 计费**分开记账**，但现有聚星 SaaS 用户可获得每月赠送额度。实现上应作为独立 grant 写入 credit ledger，不与自助套餐账单混算。

| SaaS 层级 | 赠送口径 | 折算 Credit | 工程实现 |
|----------|---------|:-----------:|---------|
| 企业版 | `￥200 / $20 / 月` | 200 credits / 月 | 每月定时 grant，不影响套餐状态 |
| 高级企业版 | `￥500 / $50 / 月` | 500 credits / 月 | 每月定时 grant，不影响套餐状态 |

**关键要求**：
- 赠送额度单独记账，账单页显示为 `granted_credits`
- SaaS 到期后停止后续赠送，但不回收已发放额度
- 赠送额度同样受风控规则约束，不能绕开 `basic_info` / analyze / monitor 的 Credit 扣减
...
Credit 额度制让批量搬取在经济上不可行。默认保护搜索搬取 100,000 个 creator 也有明确成本下限，含基础信息的大批量拉取则直接进入更高风控线；普通搜索只返回内部 `creator_id`，无法直接低价导出达人库。
...
- **实现层级**：Shell 层中间件（非 Core 层）——Core 层返回完整数据，Shell 层根据 `result_mode` / `modules` / 计费上下文过滤字段
...
# track_performance
nox track crt_abc123 --window 15d
nox track crt_abc123 --metrics followers,views,engagement --window 15d
...
| 8 | **定价预览** | 4 档简化展示（Free `￥0 / $0` / Starter `￥290 / $29` / Pro `￥990 / $99` / Growth `￥1,990 / $199`）+ "View full pricing" 链接 | Firecrawl: Pricing Teaser |
...
授权成功 → 自动创建账号（取 OAuth 返回的邮箱 + 头像 + 姓名）
  ↓
跳转 Dashboard：API Key（一键复制）+ 10 credits + "Next: Set up your AI assistant" 引导
```
...
```
         ...
         ￥0 / $0    ￥290 / $29/mo    ￥990 / $99/mo   ￥1,990 / $199/mo Custom
         10 credits  200/mo            800/mo           1,800/mo         定制
         (一次性)
...
|---------|------|---------|-----|--------|
| Credits | 10（一次性） | 200/月 | 800/月 | 1,800/月 |
...
3. Credit 用完了会怎样？（Starter / Pro / Growth 可超额按量计费；Free 停止服务）
...
| **监控趋势聚合** | 需要把频道/视频数据按 15 天监控周期聚合 | Day 1 必须 | 对频道和视频数据做时间窗聚合 + benchmark 计算 |
...
| `CreatorTrendService` | track_performance | 频道趋势 + 视频表现数据 | 聚合 15 天视图，生成 benchmark |
...
| **趋势/基准数据聚合** | track_performance | 中 | 需确认聚星频道/视频趋势数据能稳定聚合为 15 天视图 |
...
interface CreatorDetail extends CreatorCandidate {
  ...
  authenticity: {
    score: number              // 0-100
    fake_follower_pct: number  // 假粉比例
    suspicious_signals: string[] // 可疑信号列表
...
| discover 单次最大返回 | 20 | 20 | 20 | 20 | 定制 |
