...
> 状态：**草稿 v0.2**
> 更新：2026-02-26
> 依赖：`01_定位与假设.md`（v0.4，原则 P1-P7）、`02_用户场景.md`（v0.2）、`../kol-api/03_API能力设计.md`（7 Tool）、`../kol-api/05_PRD.md`（记忆架构）
...
### 6.3 AgentPod 独立容器（Enterprise，内部实现）
...
## 八、结构化数据：Core 层负责（CRM = Core 层后端）

### 8.1 设计决策：kol-agent 不自建数据库（D19 已确认）
...
| 多租户 | 部署模式（对用户透明） | Enterprise 场景(§场景5) | — | 详细设计(§6)，订阅等级对齐 | ✅ | 部署模式不暴露给用户 |
...
*下一步：05_UX 设计（Dashboard + Chat + 审批流的交互详设，渐进放权 L1/L2/L3 在 UI 上的具体呈现）。初步方案先行，使用中迭代。*
