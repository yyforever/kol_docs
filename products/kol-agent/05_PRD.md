...
> 状态：**草稿 v0.3**（v0.3：AgentPod 规模化编排层设计 §3.1.5）
> 更新：2026-03-06
> 依赖：`01_定位与假设.md`（v0.6）、`02_用户场景.md`（v0.2）、`03_产品设计.md`（v0.5）、`04_定价与商业模式.md`（v0.2）
...
#### 3.1.5 AgentPod 规模化编排（20+ 品牌）

> 2026-03-06 新增。当 Enterprise 品牌数超过 20 个，手动管理 AgentPod 不可持续。需要一个编排控制面统一管理 Agent 实例的生命周期。

**行业背景**

2026 年 3 月，OpenClaw 生态出现多个多实例管理方案：
- **OpenClaw 官方 Agents Plane**（GitHub Issue #17299）：提出 `openclaw planes` CLI，每 Agent 独立 VM/容器 + 零信任隔离。设计完整但尚未进入开发。
- **KiloClaw**（Kilo AI，GitLab 联创投资）：基于 Fly.io 的多租户托管 OpenClaw，已 GA。双层安全代理 + 500+ 模型 + 自动重启。定价 $49/agent/月。
- **SecureClaw**：开源安全加固层，对齐 OWASP/MITRE ATLAS/CSA 框架。

这些方案验证了多实例 OpenClaw 管理的市场需求和技术可行性。我们的编排层参考其设计，但自建以保持完全控制。

**编排控制面架构**

```
┌──────────────────────────────────────────────────────────┐
│  编排控制面（kol-agent Orchestrator）                       │
│                                                           │
│  ┌─────────┐  ┌───────────┐  ┌──────────┐  ┌──────────┐ │
│  │ 生命周期  │  │ 安全代理层  │  │ 密钥管理  │  │ 监控计费  │ │
│  │ 管理器   │  │（Gateway）  │  │（Vault）  │  │ 模块     │ │
│  └────┬────┘  └─────┬─────┘  └────┬─────┘  └────┬─────┘ │
│       │             │             │              │        │
├───────┼─────────────┼─────────────┼──────────────┼────────┤
│       │             │             │              │        │
...
│  └─────────┘   └─────────┘  └─────────┘                  │
│                                                           │
│  共享：容器网络、模型网关、日志聚合、计费系统                    │
└──────────────────────────────────────────────────────────┘
```

**四个核心模块**

**① 生命周期管理器** — 品牌 Agent 的创建/销毁/升级

一键创建品牌 Agent 实例，自动注入品牌专属配置：

| 创建时自动注入 | 来源 |
|--------------|------|
| SOUL.md | 基础版（默认）或品牌自定义 |
| brand_profile.md | Onboarding 收集的品牌信息 |
| AGENTS.md + skills/ | 共享达人营销知识库 |
| openclaw.json | 通道配置 + 模型 + Tool 注册 |
| MEMORY.md | 空初始化 |

管理操作：
- 创建/销毁品牌 Agent
- 批量升级 OpenClaw 版本（滚动更新，逐个 Pod）
- 实例状态查询（运行中/异常/已停止）
- 品牌数据导出（容器打包迁移）

**② 安全代理层** — 流量拦截与品牌隔离（参考 KiloClaw 双层 proxy）

```
外部请求 → API Gateway → 认证/限流/路由 → AgentPod
AgentPod 出站 → Sidecar Proxy → endpoint allowlisting → 外部服务
```

- **API Gateway**（Traefik / Kong / APISIK）：品牌认证、请求限流、路由到对应 AgentPod
- **Sidecar Proxy**：拦截 Agent 出站流量，只允许访问 kol-api + 授权邮件服务
- **凭证注入**（参考 IronClaw）：品牌的邮件密码/平台 API key 由 Sidecar 注入，Agent 侧不接触明文密钥

安全原则（对齐 OpenClaw Agents Plane #17299 零信任模型）：
- Agent 间网络不互通
- Agent 不可访问其他品牌的密钥和数据
- 管理员可查看日志/指标，但不可查看品牌密钥明文
- 所有访问可审计
...
| 密钥类型 | 存储 | 注入方式 |
|---------|------|---------|
| LLM API key | 统一模型网关（共享或品牌自带） | 网关代理 |
| 品牌邮件凭证 | Vault / 云 KMS（per-brand 前缀隔离） | Sidecar 注入 |
| 平台 API key | Vault / 云 KMS | Sidecar 注入 |
| 聚星内部 token | 编排层统一管理 | 环境变量 |

**④ 监控与计费** — per-brand 可观测性

| 维度 | 采集方式 | 用途 |
|------|---------|------|
| 健康状态 | liveness/readiness probe，异常自动重启 | 运维 |
| 资源用量 | CPU/内存 per-Pod | 容量规划 |
| Token 消耗 | 模型网关统计 per-brand | 成本追踪 |
| Agent 日志 | stdout → 日志聚合系统，按 brand label 过滤 | 排障 |
| 业务指标 | Campaign 数/邮件量/谈判轮次 per-brand | 产品分析 |
| 告警 | Agent 异常重启、token 超预算、凭证过期 | 及时响应 |

**容器编排选型：分阶段演进**

底层都是 Docker 容器，区别在于管理方式：

| 阶段 | 编排方式 | 适用规模 | 运维复杂度 |
|------|---------|---------|----------|
| **Phase 1** | Docker + 脚本 + 现有 AgentPod Control Plane | < 20 品牌 | 低 |
| **Phase 2** | 容器编排平台（K8s 或同类） | 20-500 品牌 | 中 |

> Phase 1 复用 §3.1.3 已有的 Control Plane + Docker API + Traefik 基础设施。Phase 2 的编排平台选型（K8s / ECS / 其他）在实施时由研发评估决定，不影响产品设计。迁移成本低——底层容器不变，只是管理层升级。

**规模化资源估算**

| 规模 | Pod 数 | 总 RAM | 服务器 | 月成本（计算） | 月成本（含推理） |
|------|-------|--------|-------|-------------|--------------|
| 20 品牌 | 20 | ~5GB | 1 台 8GB | ~$100-300 | ~$500-2,000 |
| 100 品牌 | 100 | ~25GB | 3-4 台 8GB | ~$500-1,500 | ~$2,500-10,000 |
| 500 品牌 | 500 | ~125GB | 集群 | ~$2,500-7,500 | ~$12,500-50,000 |

> 推理成本取决于品牌活跃度和模型选择。Starter 品牌日均 token 消耗远低于 Enterprise。

**与 §3.1.2-3.1.3 的关系**

- §3.1.2（共享实例）面向 Starter/Pro，不需要编排层——单实例多品牌 context 切换
- §3.1.3（AgentPod）面向 Enterprise，定义了单个 Pod 的内部结构
- 本节（§3.1.5）面向 **Enterprise 规模化**，定义了如何管理 20+ 个 AgentPod 的编排基础设施

三者是递进关系：共享模式 → 独立容器 → 规模化编排。
...
| D36 | AgentPod 规模化编排 | 自建编排控制面（生命周期+安全代理+密钥管理+监控计费），参考 KiloClaw 架构 + OpenClaw Agents Plane 设计。容器编排分阶段：Phase 1 Docker / Phase 2 K8s 或同类 | 05 | ✅ |
...
| 多租户 | §8 概述 | Enterprise 场景 | §七 引用 | — | §3.1 详设 + §3.1.5 规模化编排 | ✅ |
